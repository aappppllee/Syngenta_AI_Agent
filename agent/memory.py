...
from langchain.memory import ConversationBufferWindowMemory, ConversationSummaryBufferMemory, BaseChatMemory
...
class ConversationMemoryManager:
    def __init__(self, memory_config: MemoryConfig, llm_config: Optional[LLMConfig] = None):
        ...
        self._sessions_memory_wrappers: Dict[str, BaseChatMemory] = {} 
        ...
        elif self.config.buffer_type == "summary_buffer" and not self.llm_config:
    ...
    def _create_new_memory_wrapper(self, session_id: str) -> BaseChatMemory:
        ...
        # Determine buffer type based on config
    ...
    def get_session_memory(self, session_id: str) -> BaseChatMemory:
    ...
    async def save_interaction(self, session_id: str, user_query: str, ai_response_text: str):
        ...
        try:
            ...
            if hasattr(memory_wrapper, 'asave_context'):
                ...
                sql_history: SQLChatMessageHistory = memory_wrapper.chat_memory
                loop = asyncio.get_event_loop()
                # SQLChatMessageHistory methods are synchronous, run in executor
                ...
                # Fallback for purely in-memory if not SQL backed and no async save
                # This path should ideally not be hit if config.type is "sql_chat_history"
    ...
    def clear_session_memory(self, session_id: str):
        ...
        if session_id in self._sessions_memory_wrappers:
            ...
            if hasattr(memory_wrapper, 'chat_memory') and isinstance(memory_wrapper.chat_memory, SQLChatMessageHistory):
                try:
                    # SQLChatMessageHistory.clear() is a synchronous method.
        ...
        else: # If not in cache, but might exist in DB (e.g. after app restart)
            logger.warning(f"No memory wrapper found in cache for session_id '{session_id}' to clear.")
            if self.config.type == "sql_chat_history": # Attempt direct DB clear
                try:
                    sql_history_direct = self._create_sql_chat_history_instance(session_id)
                    sql_history_direct.clear() # Sync call
